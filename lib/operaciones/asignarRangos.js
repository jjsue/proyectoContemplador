...
const asignarRangosFn = function (rangoInt, operacionHabilidad, raza, nivel, habilidades) {
    ...
    while (totalPoints !== 0) {
        ...
        if (whereToPut < 80 && habilidades[claseaPosition][1] < nivel + 3){
            habilidades[claseaPosition][1]++;
            totalPoints--;
        } else {
            ...
            if (habilidades[selectedPosition][2] && habilidades[selectedPosition][1] < nivel + 3){
                console.log(habilidades[selectedPosition][1]);
                console.log(habilidades[selectedPosition][1] <= nivel + 3);
                habilidades[selectedPosition][1]++ //Es de clase y automaticamente sumamos y continuamos el bucle
                totalPoints--;
                continue;
            } else if(totalPoints >= 2 && habilidades[selectedPosition][1] <= Math.floor((nivel + 3) / 2)){
